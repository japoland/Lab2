...
var url='mongodb://student:student@cs490-shard-00-00-btbrn.mongodb.net:27017,cs490-shard-00-01-btbrn.mongodb.net:27017,cs490-shard-00-02-btbrn.mongodb.net:27017/test?ssl=true&replicaSet=CS490-shard-0&authSource=admin&retryWrites=true&w=majority';
...
var deleteDocument = function(db, data, callback){
    //delete
    db.collection('users').deleteOne({"_id": ObjectID(data.params.toBeDeleted_id)}, function(err, result){
...
var updateDocument = function(db, data, callback){
    db.collection('users').updateOne({"_id": ObjectID(data._id)}, {$set: {"password": data.password, "firstName": data.firstName, "lastName": data.lastName}}, function(err, result){
        console.log(data);
    if(err)
        {
